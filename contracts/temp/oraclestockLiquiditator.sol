...
contract stockLiquiditator is  usingProvable
{
    using SafeMath for uint256;
    ...
    ERC20 internal PoolToken;
    ERC20 internal StockToken;
    
    uint256 public StocktoDAI_rate;
    ...
    constructor (address StockPoolTokenAddr,address DAITokenAddr,address StockTokenAddr)  public  {
        owner = msg.sender;
        require(StockTokenAddr != address(0), "StockToken is the zero address");
        require(StockPoolTokenAddr != address(0), "StockPoolToken is the zero address");
    ...
    function updateDAIValuationCap() public payable returns(bool){
        if (provable_getPrice("URL") > address(this).balance) {
            ...
            provable_query("URL", "json(https://oracleprov.herokuapp.com/get).price");
            return true; 
        }
    }
    ...
    function __callback(bytes32 myid,string memory  result) public {
        ...
        StocktoDAI_rate = parseInt(result);
    }
    
    
    function StockPoolTokenAddress() public view returns (address) {
    ...
    function poolTokenToMint(uint256 inputDAI) internal returns (uint256 mintablePoolToken) {
    ...
    function mintPoolToken(uint256 DAIAmount) external {    //insert only dai & minting pool token
        ...
        uint256 poolTokens = poolTokenToMint(DAIAmount);

        require(ERC20Mintable(address(StockPoolTokenAddress())).mint(sender, poolTokens),"Minting failed"); //Minting Stock Pool Token
    }
    
    function burnPoolToken(uint256 tokenAmount) external {  
        address sender= msg.sender;
        _preValidateData(sender,tokenAmount);
        
        require(PoolToken.balanceOf(sender)>=tokenAmount,"Insufficient Balance");
        uint256 burnallowance = PoolToken.allowance(sender, address(this));
        require(burnallowance>=tokenAmount,"Insufficient Burn Allowance");
        
        uint256 daiToRedeem=redeemableDai(tokenAmount);
        
        ERC20Burnable(address(StockPoolTokenAddress())).burnFrom(sender, tokenAmount);  //Burning Pool Token
        ...
        uint256 outputDAIAmount=daiToRedeem.sub(stockTokenDAIValuation());// calculate dai amount to be return
        StockToken.transfer(sender,allStockToken);
        DAIToken.transfer(sender,outputDAIAmount);
        }
    }
    
    function redeemStockToken(uint256 tokenAmount) external{
        address sender= msg.sender;
        _preValidateData(sender,tokenAmount);
        
        require(StockToken.balanceOf(sender)>=tokenAmount,"Insufficient Balance");
        uint256 burnallowance = StockToken.allowance(sender, address(this));
        require(burnallowance>=tokenAmount,"Insufficient Burn Allowance");
